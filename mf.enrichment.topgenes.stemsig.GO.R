...
# get go ids of MF only:
go <- as.data.frame(org.Hs.egGO2ALLEGS)
mfid <- go$go_id[go$Ontology=="MF"]
mfid <- unique(mfid)
...
# list with human mf ids and corresponding genes:
xx.mf <- xx.GO[mfid]
# to get gene universe for Hs MF
allgenes <- unique(unlist(xx.mf))
...
# in geneIds use "rank.genes[rank.genes %in% allgenes]" to use from the genes we want to analyze those that have a corresponding GO MF Hs slim term (are annotated)
# annotation - annotation package used(for the function to know what organism we are using)
# ontology - specifies the GO ontology: is it biological process (MF), molecular function (MF) or celullar compartment (CC) we're analyzing
...
paramsGO <- new("GOHyperGParams", geneIds =top.rank[top.rank %in% allgenes], universeGeneIds = allgenes, annotation = "org.Hs.eg.db", ontology = "MF", testDirection = "over", conditional=T, pvalueCutoff=0.05, categorySubsetIds = mfid) # all parameters are explained just above
...
go1 <- subset(tgo,tgo$GOMFID%in%mfid) # to have only go.slim terms in the table
go1 <- data.frame(go1[, 1:2], FDR = p.adjust(go1[, 2], method = "fdr", n=length(mfid)),go1[, 3:7]) # !IMPORTANT: there are some no significant GO IDs that are not shown even after selecting p-value threshold as 1.1. So the remaining GO terms although not in the table will have to be included in the p-value adjustment calculation: so we added n=length(mfid)
...
onto.overGO.mf.h <- data.frame(go1,entrez.id = entrez.genesGO,gene.symbol = sym.genesGO)
...
onto.overGO.mf.h<-data.frame(onto.overGO.mf.h[,1:8],GO.description=GOIDs.Description[,2],onto.overGO.mf.h[,9:ncol(onto.overGO.mf.h)])
# order data frame from most signifficant term (lower FDR) to less signifficant term:
ord <- order(onto.overGO.mf.h$FDR)      
onto.overGO.mf.h1 <- onto.overGO.mf.h[ord,]
# to select the terms with signifficant FDR:
signifh <- onto.overGO.mf.h1[onto.overGO.mf.h1$FDR < 0.05,]
write.csv(signifh, "C:/Users/Tania/Internship/R/Results/enrichment.top/stemness_signatures/GOMF/mf.human.top.stemness.csv", row.names=F)
...
# "GOHyperGParams" is a parameter class (group of parameters) for analyzing for the GO.
# There're others: the KEGGHyperGParams and PFAMHyperGParams
# in geneIds use "rank.genes[rank.genes %in% allgenes]" to use from the genes we want to analyze those that have a corresponding GO MF Mm slim term (are annotated)
# annotation - annotation package used(for the function to know what organism we are using)
# ontology - specifies the GO ontology: is it biological process (MF), molecular function (MF) or celullar compartment (CC) we're analyzing
# test direction:  detects over or under represented GO terms
...
mapped_genesGO <- mappedkeys(x.2GO) 
entrez2symGO <- as.vector(as.list(x.2GO[mapped_genesGO])) #it's still a list
paramsGO <- new("GOHyperGParams", geneIds =top.rank[top.rank %in% allgenes], universeGeneIds = allgenes, annotation = "org.Mm.eg.db", ontology = "MF", testDirection = "over", conditional=T, pvalueCutoff=0.05, categorySubsetIds = mfid) # all parameters are explained just above
hypGO <- hyperGTest(paramsGO)
tgo <- summary(hypGO, pvalue = 1.1) # only terms with p-value less than the cutoff specified will be shown in the table.
# we use 1.1 to include p-value 1. we include p-value 1 cause 1st we need to calculate adjusted p-value before applying the cutoff and for using these tables for the heatmap
# note: table shows GO terms with size 0: I think it's because some child terms are not considered with conditional=T (when we do the analysis with conditional F and without pvalueCutoff parameter set we don't get GO terms with size 0)                                  
...
# merge onto.over with gene ids and gene symbols for each GO term
onto.overGO.mf.m <- data.frame(go1,entrez.id = entrez.genesGO,gene.symbol = sym.genesGO)
# find correponding term explanation for GOIDs
GOIDs<-go1[,1]
GOIDs.Description<-select(GO.db, keys=GOIDs, columns="DEFINITION", keytype="GOID")
# merge onto.over.m with GO term explanation
onto.overGO.mf.m<-data.frame(onto.overGO.mf.m[,1:8],GO.description=GOIDs.Description[,2],onto.overGO.mf.m[,9:ncol(onto.overGO.mf.m)])
# order data frame from most signifficant term (lower FDR) to less signifficant term:
ord <- order(onto.overGO.mf.m$FDR)      
onto.overGO.mf.m1 <- onto.overGO.mf.m[ord,]
# to select the terms with signifficant FDR:
signifm <- onto.overGO.mf.m1[onto.overGO.mf.m1$FDR < 0.05,]
write.csv(signifm, "C:/Users/Tania/Internship/R/Results/enrichment.top/stemness_signatures/GOMF/mf.mouse.top.stemness.csv", row.names=F)

### Bar plots (human and mouse together in same barplot)
setwd("C:/Users/Tania/Internship/R/Results/enrichment.top/stemness_signatures/GOMF")
rank.human <- read.csv("mf.human.top.stemness.csv", header=T, stringsAsFactors=F)
rank.human <- rank.human[,c(8,3)]
rank.mouse <- read.csv("mf.mouse.top.stemness.csv", header=T, stringsAsFactors=F)
...
# NOTE!!!: as there are some MF terms for mouse that don't exist in human and vice-versa
...
png(file = "C:/Users/Tania/Internship/R/Results/enrichment.top/stemness_signatures/GOMF/mf.enrich.bothmousehuman.top.stemness.png", width = 25*600, height = 10*600, res = 600)#export file to png
...
barplot(botha, horiz = T, beside=T, las=1, space = c(1/ncol(botha),5/ncol(botha)), col=colorRampPalette(c("forestgreen","red"))(n=2), cex.names=4.9, axes = F, xlim = c(0,4)) # both axis have to be set false so that then we can overlaigh a axis with parameters we want
# to use main title with its corresponding parameters (for ex. size) we have to set the title appart from barplot:
# title(main = list("Molecular Function - both mouse and human", cex = 5))
#par(cex.axis=1, cex.lab=3, cex.main=500)
axis(side=1, at=seq(from=0,to = 4, by=2), lwd=5, tck = -0.02, labels=NA, line = 2) # this is for the ticks only in x axis. that's why lables=NA.side - an integer indicating the side of the graph to draw the axis (1=bottom, 2=left, 3=top, 4=right); at - a numeric vector indicating where tick marks should be drawn; lwd - line width;
axis(side=1, at=seq(from=0,to = 4, by=2), lwd=0, cex.axis=5, line = 5) # this is for the numbering only in x axis. that's why lwd=0.
title(xlab=expression(-log[10](`adj.p-value`)), cex.lab =5.5, line = 13) # adds x axis label
dev.off()

tiff(file = "C:/Users/Tania/Internship/R/Results/enrichment.top/stemness_signatures/GOMF/mf.enrich.bothmousehuman.top.stemness.tif", width = 25*600, height = 10*600, res = 600)#export file to png
par(mar=c(14, max(nchar(colnames(botha)))+30, 15,10))
#barplot(botha, horiz = T, beside=T, las=1, space = c(15/ncol(botha),45/ncol(botha)), col=colorRampPalette(c("forestgreen","red"))(n=2), cex.names=4.9, legend.text=T, args.legend=list(x=0, y=0, cex = 3), axes = F, xlim = c(0,20)) # both axis have to be set false so that then we can overlaigh a axis with parameters we want
barplot(botha, horiz = T, beside=T, las=1, space = c(1/ncol(botha),5/ncol(botha)), col=colorRampPalette(c("forestgreen","red"))(n=2), cex.names=4.9, axes = F, xlim = c(0,4)) # both axis have to be set false so that then we can overlaigh a axis with parameters we want
# to use main title with its corresponding parameters (for ex. size) we have to set the title appart from barplot:
...
# tiff(file = "C:/Users/Tania/Internship/R/Results/enrichment.top/stemness_signatures/GOMF/mf.enrich.bothmousehuman.top.stemness.tiff", width = 35*300, height = 10*300, res = 300)#export file to png
# par(mar=c(10, max(nchar(colnames(botha)))+20, 10,5))
# barplot(botha, horiz = T, beside=T, las=1, space = c(1/ncol(botha),2/ncol(botha)), col=colorRampPalette(c("forestgreen","red"))(n=2), cex.names=4, legend.text=T, args.legend=list(cex = 3), axes = F, xlim = c(0,4)) # both axis have to be set false so that then we can overlaigh a axis with parameters we want
# # to use main title with its corresponding parameters (for ex. size) we have to set the title appart from barplot:
# # title(main = list("Molecular Function - both mouse and human", cex = 5))
# axis(side=1, at=seq(from=0,to = 4, by=2), lwd=4, tck = -0.03, labels=NA, line = 1) # this is for the ticks only in x axis. that's why lables=NA.side - an integer indicating the side of the graph to draw the axis (1=bottom, 2=left, 3=top, 4=right); at - a numeric vector indicating where tick marks should be drawn; lwd - line width;
# axis(side=1, at=seq(from=0,to = 4, by=2), lwd=0, cex=5, cex.axis=3.5, line = 3) # this is for the numbering only in x axis. that's why lwd=0.
...
png(file = "C:/Users/Tania/Internship/R/Results/enrichment.top/stemness_signatures/GOMF/mf.enrich.humanonly.top.stemness.png", width = 25*600, height = 10*600, res = 600)#export file to png
par(mar=c(10, max(nchar(colnames(mouseonlya)))+5, 5,15))
barplot(humanonlya, horiz = T, beside=T, las=1, space = c(10/ncol(humanonlya),10/ncol(humanonlya)), col=colorRampPalette(c("red"))(n=1), cex.names=2, axes=F, xlim=c(0,8)) # both axis have to be set false so that then we can overlaigh a axis with parameters we want
# to use main title with its corresponding parameters (for ex. size) we have to set the title appart from barplot:
# title(main = list("Biological Processes - human only", cex = 5))
axis(side=1, at=seq(from=0,to = 8, by=4), lwd=6, tck = -0.01, labels=NA, line = 0) # this is for the ticks only in x axis. that's why lables=NA.side - an integer indicating the side of the graph to draw the axis (1=bottom, 2=left, 3=top, 4=right); at - a numeric vector indicating where tick marks should be drawn; lwd - line width;
axis(side=1, at=seq(from=0,to = 8, by=4), lwd=0, cex.axis=2, line = 1) # this is for the numbering only in x axis. that's why lwd=0.
title(xlab=expression(-log[10](`adj.p-value`)), cex.lab = 2, line = 6) # adds x axis label
dev.off()

tiff(file = "C:/Users/Tania/Internship/R/Results/enrichment.top/stemness_signatures/GOMF/mf.enrich.humanonly.top.stemness.tif", width = 25*600, height = 10*600, res = 600)#export file to png
par(mar=c(10, max(nchar(colnames(mouseonlya)))+5, 5,15))
barplot(humanonlya, horiz = T, beside=T, las=1, space = c(10/ncol(humanonlya),10/ncol(humanonlya)), col=colorRampPalette(c("red"))(n=1), cex.names=2, axes=F, xlim=c(0,8)) # both axis have to be set false so that then we can overlaigh a axis with parameters we want
# to use main title with its corresponding parameters (for ex. size) we have to set the title appart from barplot:
# title(main = list("Biological Processes - human only", cex = 5))
...
# tiff(file = "C:/Users/Tania/Internship/R/Results/enrichment.top/stemness_signatures/GOMF/mf.enrich.humanonly.top.stemness.tiff", width = 50*300, height = 30*300, res = 300)#export file to png
# par(mar=c(23, max(nchar(colnames(humanonlya)))+60, 10, 10))
# barplot(humanonlya, horiz = T, beside=T, las=1, space = c(5/ncol(humanonlya),5/ncol(humanonlya)), col=colorRampPalette(c("red"))(n=1), cex.names=5, axes=F, xlim=c(0,8)) # both axis have to be set false so that then we can overlaigh a axis with parameters we want
# # to use main title with its corresponding parameters (for ex. size) we have to set the title appart from barplot:
# # title(main = list("Molecular Function - human only", cex = 5))
# axis(side=1, at=seq(from=0,to = 8, by=2), lwd=7, tck = -0.02, labels=NA, line = -1) # this is for the ticks only in x axis. that's why lables=NA.side - an integer indicating the side of the graph to draw the axis (1=bottom, 2=left, 3=top, 4=right); at - a numeric vector indicating where tick marks should be drawn; lwd - line width;
# axis(side=1, at=seq(from=0,to = 8, by=2), lwd=0, cex=5, cex.axis=4, line = 3) # this is for the numbering only in x axis. that's why lwd=0.
# title(xlab=expression(-log[10](`adj.p-value`)), cex.lab = 5.5, line = 12) # adds x axis label
# dev.off()
...
png(file = "C:/Users/Tania/Internship/R/Results/enrichment.top/stemness_signatures/GOMF/mf.enrich.mouseonly.top.stemness.png", width = 25*600, height = 10*600, res = 600)#export file to png
par(mar=c(10, max(nchar(colnames(mouseonlya)))+5, 5,15))
barplot(mouseonlya, horiz = T, beside=T, las=1, space = c(10/ncol(mouseonlya),10/ncol(mouseonlya)), col=colorRampPalette(c("forestgreen"))(n=1), cex.names=2, axes=F, xlim=c(0,8)) # both axis have to be set false so that then we can overlaigh a axis with parameters we want
# to use main title with its corresponding parameters (for ex. size) we have to set the title appart from barplot:
# title(main = list("Molecular Function - mouse only", cex = 5))
axis(side=1, at=seq(from=0,to = 8, by=4), lwd=6, tck = -0.02, labels=NA, line = 0) # this is for the ticks only in x axis. that's why lables=NA.side - an integer indicating the side of the graph to draw the axis (1=bottom, 2=left, 3=top, 4=right); at - a numeric vector indicating where tick marks should be drawn; lwd - line width;
axis(side=1, at=seq(from=0,to = 8, by=4), lwd=0, cex.axis=2, line = 1) # this is for the numbering only in x axis. that's why lwd=0.
title(xlab=expression(-log[10](`adj.p-value`)), cex.lab = 2, line = 6) # adds x axis label
dev.off()

tiff(file = "C:/Users/Tania/Internship/R/Results/enrichment.top/stemness_signatures/GOMF/mf.enrich.mouseonly.top.stemness.tif", width = 25*600, height = 10*600, res = 600)#export file to png
par(mar=c(10, max(nchar(colnames(mouseonlya)))+5, 5,15))
barplot(mouseonlya, horiz = T, beside=T, las=1, space = c(10/ncol(mouseonlya),10/ncol(mouseonlya)), col=colorRampPalette(c("forestgreen"))(n=1), cex.names=2, axes=F, xlim=c(0,8)) # both axis have to be set false so that then we can overlaigh a axis with parameters we want
# to use main title with its corresponding parameters (for ex. size) we have to set the title appart from barplot:
# title(main = list("Molecular Function - mouse only", cex = 5))
...
# tiff(file = "C:/Users/Tania/Internship/R/Results/enrichment.top/stemness_signatures/GOMF/mf.enrich.mouseonly.top.stemness.tiff", width = 50*300, height = 30*300, res = 300)#export file to png
# par(mar=c(17, max(nchar(colnames(mouseonlya)))+80, 5, 10))
# barplot(mouseonlya, horiz = T, beside=T, las=1, space = c(30/ncol(mouseonlya),30/ncol(mouseonlya)), col=colorRampPalette(c("forestgreen"))(n=1), cex.names=5, axes=F, xlim = c(0,8)) # both axis have to be set false so that then we can overlaigh a axis with parameters we want
# # to use main title with its corresponding parameters (for ex. size) we have to set the title appart from barplot:
# #title(main = list("Molecular Function - mouse only", cex = 5))
# axis(side=1, at=seq(from=0,to = 8, by=4), lwd=7, tck = -0.02, labels=NA, line = -1) # this is for the ticks only in x axis. that's why lables=NA.side - an integer indicating the side of the graph to draw the axis (1=bottom, 2=left, 3=top, 4=right); at - a numeric vector indicating where tick marks should be drawn; lwd - line width;
# axis(side=1, at=seq(from=0,to = 8, by=4), lwd=0, cex=5, cex.axis=3.5, line = 2) # this is for the numbering only in x axis. that's why lwd=0.
# title(xlab=expression(-log[10](`adj.p-value`)), cex.lab = 5.5, line = 10) # adds x axis label
# dev.off()
...
# extra.human <- subset(onto.overGO.mf.h1[,c(8,3)],onto.overGO.mf.h1$Term%in%NAs.human) 
# extra.mouse <- subset(onto.overGO.mf.m1[,c(8,3)],onto.overGO.mf.m1$Term%in%NAs.mouse) 
# # put column FDR with the indication if it's from mouse or human
