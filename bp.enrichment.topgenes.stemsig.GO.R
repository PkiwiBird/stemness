...
# get go ids of BP only:
go <- as.data.frame(org.Hs.egGO2ALLEGS)
bpid <- go$go_id[go$Ontology=="BP"]
bpid <- unique(bpid)
...
# list with human bp ids and corresponding genes:
xx.bp <- xx.GO[bpid]
# to get gene universe for Hs BP
allgenes <- unique(unlist(xx.bp))
...
# in geneIds use "rank.genes[rank.genes %in% allgenes]" to use from the genes we want to analyze those that have a corresponding GO BP Hs slim term (are annotated)
# annotation - annotation package used(for the function to know what organism we are using)
# ontology - specifies the GO ontology: is it biological process (BP), molecular function (MF) or celullar compartment (CC) we're analyzing
...
paramsGO <- new("GOHyperGParams", geneIds =top.rank[top.rank %in% allgenes], universeGeneIds = allgenes, annotation = "org.Hs.eg.db", ontology = "BP", testDirection = "over", conditional=T, pvalueCutoff=0.05, categorySubsetIds = bpid) # all parameters are explained just above
...
go1 <- subset(tgo,tgo$GOBPID%in%bpid) # to confirm we only have go bp terms in the table
go1 <- data.frame(go1[, 1:2], FDR = p.adjust(go1[, 2], method = "fdr", n=length(bpid)),go1[, 3:7]) # !IMPORTANT: there are some no significant GO IDs that are not shown even after selecting p-value threshold as 1.1. So the remaining GO terms although not in the table will have to be included in the p-value adjustment calculation: so we added n=length(bpid)
...
onto.overGO.bp.h <- data.frame(go1,entrez.id = entrez.genesGO,gene.symbol = sym.genesGO)
...
onto.overGO.bp.h<-data.frame(onto.overGO.bp.h[,1:8],GO.description=GOIDs.Description[,2],onto.overGO.bp.h[,9:ncol(onto.overGO.bp.h)])
# order data frame from most signifficant term (lower FDR) to less signifficant term:
ord <- order(onto.overGO.bp.h$FDR)      
onto.overGO.bp.h1 <- onto.overGO.bp.h[ord,]
# to select the terms with signifficant FDR:
signifh <- onto.overGO.bp.h1[onto.overGO.bp.h1$FDR < 0.05,]
write.csv(signifh, "C:/Users/Tania/Internship/R/Results/enrichment.top/stemness_signatures/GOBP/bp.human.top.stemness.csv", row.names=F)
...
# "GOHyperGParams" is a parameter class (group of parameters) for analyzing for the GO.
# There're others: the KEGGHyperGParams and PFAMHyperGParams
# in geneIds use "rank.genes[rank.genes %in% allgenes]" to use from the genes we want to analyze those that have a corresponding GO BP Mm slim term (are annotated)
# annotation - annotation package used(for the function to know what organism we are using)
# ontology - specifies the GO ontology: is it biological process (BP), molecular function (MF) or celullar compartment (CC) we're analyzing
# test direction:  detects over or under represented GO terms
...
mapped_genesGO <- mappedkeys(x.2GO) 
entrez2symGO <- as.vector(as.list(x.2GO[mapped_genesGO])) #it's still a list
paramsGO <- new("GOHyperGParams", geneIds =top.rank[top.rank %in% allgenes], universeGeneIds = allgenes, annotation = "org.Mm.eg.db", ontology = "BP", testDirection = "over", conditional=T, pvalueCutoff=0.05, categorySubsetIds = bpid) # all parameters are explained just above
hypGO <- hyperGTest(paramsGO)
tgo <- summary(hypGO, pvalue = 1.1) # only terms with p-value less than the cutoff specified will be shown in the table.
# we use 1.1 to include p-value 1. we include p-value 1 cause 1st we need to calculate adjusted p-value before applying the cutoff and for using these tables for the heatmap
# note: table shows GO terms with size 0: I think it's because some child terms are not considered with conditional=T (when we do the analysis with conditional F and without pvalueCutoff parameter set we don't get GO terms with size 0)                                  
# selection of signifficant terms for the outputed tables is done downstream
go1 <- subset(tgo,tgo$GOBPID%in%bpid) # to have only go.slim terms in the table
go1 <- data.frame(go1[, 1:2], FDR = p.adjust(go1[, 2], method = "fdr", n=length(bpid)),go1[, 3:7]) # !IMPORTANT: there are some no significant GO IDs that are not shown even after selecting p-value threshold as 1.1. So the remaining GO terms although not in the table will have to be included in the p-value adjustment calculation: so we added n=length(bpid)
# we may have a small number of FDR (significant) and still have a small number of counts(small number of genes in the gene set with that GO term)       
# because p-values (which are afterwards transformed by FDR into q-values)compare the difference between two values with the variance (i.e. the range over which the values fall), even with a small number of counts we may have a significant difference when the variance is small 
# besides having a significant value (small FDR) we may also want only the GO terms with a high number of counts, to further narrow the number of GO terms found
...
# merge onto.over with gene ids and gene symbols for each GO term
onto.overGO.bp.m <- data.frame(go1,entrez.id = entrez.genesGO,gene.symbol = sym.genesGO)
# find correponding term explanation for GOIDs
GOIDs<-go1[,1]
GOIDs.Description<-select(GO.db, keys=GOIDs, columns="DEFINITION", keytype="GOID")
# merge onto.over.m with GO term explanation
onto.overGO.bp.m<-data.frame(onto.overGO.bp.m[,1:8],GO.description=GOIDs.Description[,2],onto.overGO.bp.m[,9:ncol(onto.overGO.bp.m)])
# order data frame from most signifficant term (lower FDR) to less signifficant term:
ord <- order(onto.overGO.bp.m$FDR)      
onto.overGO.bp.m1 <- onto.overGO.bp.m[ord,]
# to select the terms with signifficant FDR:
signifm <- onto.overGO.bp.m1[onto.overGO.bp.m1$FDR < 0.05,]
write.csv(signifm, "C:/Users/Tania/Internship/R/Results/enrichment.top/stemness_signatures/GOBP/bp.mouse.top.stemness.csv", row.names=F)

### Bar plots (human and mouse together in same barplot)
setwd("C:/Users/Tania/Internship/R/Results/enrichment.top/stemness_signatures/GOBP")
rank.human <- read.csv("bp.human.top.stemness.csv", header=T, stringsAsFactors=F)
rank.human <- rank.human[,c(8,3)]
rank.mouse <- read.csv("bp.mouse.top.stemness.csv", header=T, stringsAsFactors=F)
...
# NOTE!!!: as there are some BP terms for mouse that don't exist in human and vice-versa
...
botha <- -log10(botha)
# change the order in which two of the processes is shown (Matthias asked):
botha <- cbind(botha[,1:14],botha[,c(26,27)], botha[,17:25], botha[,c(15,16)])
# do the graph:
...
png(file = "C:/Users/Tania/Internship/R/Results/enrichment.top/stemness_signatures/GOBP/bp.enrich.bothmousehuman.top.stemness.png", width = 35*600, height = 30*600, res = 600)#export file to png
par(mar=c(10, max(nchar(colnames(botha)))+64, 15,10))
...
barplot(botha, horiz = T, beside=T, las=1, space = c(15/ncol(botha),45/ncol(botha)), col=colorRampPalette(c("forestgreen","red"))(n=2), cex.names=4.9, axes = F, xlim = c(0,20)) # both axis have to be set false so that then we can overlaigh a axis with parameters we want
# to use main title with its corresponding parameters (for ex. size) we have to set the title appart from barplot:
# title(main = list("Biological Processes - both mouse and human", cex = 5))
#par(cex.axis=1, cex.lab=3, cex.main=500)
axis(side=1, at=seq(from=0,to = 20, by=10), lwd=5, tck = -0.02, labels=NA, line = -2) # this is for the ticks only in x axis. that's why lables=NA.side - an integer indicating the side of the graph to draw the axis (1=bottom, 2=left, 3=top, 4=right); at - a numeric vector indicating where tick marks should be drawn; lwd - line width;
axis(side=1, at=seq(from=0,to = 20, by=10), lwd=0, cex.axis=5, line = 0.5) # this is for the numbering only in x axis. that's why lwd=0.
title(xlab=expression(-log[10](`adj.p-value`)), cex.lab =5.5, line = 9) # adds x axis label
dev.off()

tiff(file = "C:/Users/Tania/Internship/R/Results/enrichment.top/stemness_signatures/GOBP/bp.enrich.bothmousehuman.top.stemness.tiff", width = 35*600, height = 30*600, res = 600)#export file to png
par(mar=c(10, max(nchar(colnames(botha)))+64, 15,10))
#barplot(botha, horiz = T, beside=T, las=1, space = c(15/ncol(botha),45/ncol(botha)), col=colorRampPalette(c("forestgreen","red"))(n=2), cex.names=4.9, legend.text=T, args.legend=list(x=0, y=0, cex = 3), axes = F, xlim = c(0,20)) # both axis have to be set false so that then we can overlaigh a axis with parameters we want
barplot(botha, horiz = T, beside=T, las=1, space = c(15/ncol(botha),45/ncol(botha)), col=colorRampPalette(c("forestgreen","red"))(n=2), cex.names=4.9, axes = F, xlim = c(0,20)) # both axis have to be set false so that then we can overlaigh a axis with parameters we want
# to use main title with its corresponding parameters (for ex. size) we have to set the title appart from barplot:
...
# tiff(file = "C:/Users/Tania/Internship/R/Results/enrichment.top/stemness_signatures/GOBP/bp.enrich.bothmousehuman.top.stemness.tiff", width = 60*300, height = 30*300, res = 300)#export file to png
# par(mar=c(20, max(nchar(colnames(botha)))+45, 10,15))
# barplot(botha, horiz = T, beside=T, las=1, space = c(7/ncol(botha),17/ncol(botha)), col=colorRampPalette(c("forestgreen","red"))(n=2), cex.names=4, legend.text=T, args.legend=list(cex = 3), axes = F, xlim = c(0,20)) # both axis have to be set false so that then we can overlaigh a axis with parameters we want
# # to use main title with its corresponding parameters (for ex. size) we have to set the title appart from barplot:
# # title(main = list("Biological Processes - both mouse and human", cex = 5))
# #par(cex.axis=1, cex.lab=3, cex.main=500)
# axis(side=1, at=seq(from=0,to = 20, by=4), lwd=4, tck = -0.01, labels=NA, line = -2) # this is for the ticks only in x axis. that's why lables=NA.side - an integer indicating the side of the graph to draw the axis (1=bottom, 2=left, 3=top, 4=right); at - a numeric vector indicating where tick marks should be drawn; lwd - line width;
# axis(side=1, at=seq(from=0,to = 20, by=4), lwd=0, cex=5, cex.axis=3.5, line = 0.5) # this is for the numbering only in x axis. that's why lwd=0.
# title(xlab=expression(-log[10](`adj.p-value`)), cex.lab = 4.5, line = 7.5) # adds x axis label
# dev.off()
...
png(file = "C:/Users/Tania/Internship/R/Results/enrichment.top/stemness_signatures/GOBP/bp.enrich.humanonly.top.stemness.png", width = 30*600, height = 49*600, res = 600)#export file to png
par(mar=c(10, max(nchar(colnames(humanonlya)))+55, 5,15))
barplot(humanonlya, horiz = T, beside=T, las=1, space = c(40/ncol(humanonlya),40/ncol(humanonlya)), col=colorRampPalette(c("red"))(n=1), cex.names=4.1, axes=F, xlim=c(0,12)) # both axis have to be set false so that then we can overlaigh a axis with parameters we want
# to use main title with its corresponding parameters (for ex. size) we have to set the title appart from barplot:
...
axis(side=1, at=seq(from=0,to = 12, by=6), lwd=0, cex.axis=5, line = -3) # this is for the numbering only in x axis. that's why lwd=0.
title(xlab=expression(-log[10](`adj.p-value`)), cex.lab = 5.5, line = 6) # adds x axis label
dev.off()

tiff(file = "C:/Users/Tania/Internship/R/Results/enrichment.top/stemness_signatures/GOBP/bp.enrich.humanonly.top.stemness.tif", width = 30*600, height = 49*600, res = 600)#export file to png
par(mar=c(10, max(nchar(colnames(humanonlya)))+55, 5,15))
barplot(humanonlya, horiz = T, beside=T, las=1, space = c(40/ncol(humanonlya),40/ncol(humanonlya)), col=colorRampPalette(c("red"))(n=1), cex.names=4.1, axes=F, xlim=c(0,12)) # both axis have to be set false so that then we can overlaigh a axis with parameters we want
# to use main title with its corresponding parameters (for ex. size) we have to set the title appart from barplot:
# title(main = list("Biological Processes - human only", cex = 5))
...
# tiff(file = "C:/Users/Tania/Internship/R/Results/enrichment.top/stemness_signatures/GOBP/bp.enrich.humanonly.top.stemness.tif", width = 50*300, height = 70*300, res = 300)#export file to png
# par(mar=c(20, max(nchar(colnames(humanonlya)))+75, 20, 15))
# barplot(humanonlya, horiz = T, beside=T, las=1, space = c(24/ncol(humanonlya),24/ncol(humanonlya)), col=colorRampPalette(c("red"))(n=1), cex.names=5, axes=F, xlim=c(0,12)) # both axis have to be set false so that then we can overlaigh a axis with parameters we want
# # to use main title with its corresponding parameters (for ex. size) we have to set the title appart from barplot:
# # title(main = list("Biological Processes - human only", cex = 5))
# axis(side=1, at=seq(from=0,to = 12, by=2), lwd=7, tck = -0.02, labels=NA, line = -8) # this is for the ticks only in x axis. that's why lables=NA.side - an integer indicating the side of the graph to draw the axis (1=bottom, 2=left, 3=top, 4=right); at - a numeric vector indicating where tick marks should be drawn; lwd - line width;
# axis(side=1, at=seq(from=0,to = 12, by=2), lwd=0, cex=5, cex.axis=3.5, line = -4) # this is for the numbering only in x axis. that's why lwd=0.
# title(xlab=expression(-log[10](`adj.p-value`)), cex.lab = 5.5, line = 5.5) # adds x axis label
# dev.off()
...
# select top 100 (cause it is too many):
mouseonlya <- mouseonlya[1:100]
# give terms as names of the vector:
names(mouseonlya) <- mouseonly[1:100,1]
# reverse order of vector (so that later on the terms most enriched are shown first in barplot):
...
png(file = "C:/Users/Tania/Internship/R/Results/enrichment.top/stemness_signatures/GOBP/bp.enrich.mouseonly.top.stemness.png", width = 40*500, height = 53*500, res = 500)#export file to png
par(mar=c(10, max(nchar(colnames(mouseonlya)))+55,1,5))
barplot(mouseonlya, horiz = T, beside=T, las=1, space = c(35/ncol(mouseonlya),35/ncol(mouseonlya)), col=colorRampPalette(c("forestgreen"))(n=1), cex.names=3.6, axes=F, xlim=c(0,20)) # both axis have to be set false so that then we can overlaigh a axis with parameters we want
# to use main title with its corresponding parameters (for ex. size) we have to set the title appart from barplot:
# title(main = list("Biological Processes - human only", cex = 5))
axis(side=1, at=seq(from=0,to = 20, by=10), lwd=7, tck = -0.04, labels=NA, line = -6) # this is for the ticks only in x axis. that's why lables=NA.side - an integer indicating the side of the graph to draw the axis (1=bottom, 2=left, 3=top, 4=right); at - a numeric vector indicating where tick marks should be drawn; lwd - line width;
axis(side=1, at=seq(from=0,to = 20, by=10), lwd=0, cex.axis=5, line = -3) # this is for the numbering only in x axis. that's why lwd=0.
title(xlab=expression(-log[10](`adj.p-value`)), cex.lab = 5.5, line = 6) # adds x axis label
dev.off()

tiff(file = "C:/Users/Tania/Internship/R/Results/enrichment.top/stemness_signatures/GOBP/bp.enrich.mouseonly.top.stemness.tif", width = 40*500, height = 53*500, res = 500)#export file to png
par(mar=c(10, max(nchar(colnames(mouseonlya)))+55,1,5))
barplot(mouseonlya, horiz = T, beside=T, las=1, space = c(35/ncol(mouseonlya),35/ncol(mouseonlya)), col=colorRampPalette(c("forestgreen"))(n=1), cex.names=3.6, axes=F, xlim=c(0,20)) # both axis have to be set false so that then we can overlaigh a axis with parameters we want
# to use main title with its corresponding parameters (for ex. size) we have to set the title appart from barplot:
# title(main = list("Biological Processes - human only", cex = 5))
...
# tiff(file = "C:/Users/Tania/Internship/R/Results/enrichment.top/stemness_signatures/GOBP/bp.enrich.mouseonly.top.stemness.tiff", width = 55*300, height = 80*300, res = 300)#export file to png
# par(mar=c(20, max(nchar(colnames(humanonlya)))+140, 20, 10))
# barplot(mouseonlya, horiz = T, beside=T, las=1, space = c(30/ncol(mouseonlya),30/ncol(mouseonlya)), col=colorRampPalette(c("forestgreen"))(n=1), cex.names=5, axes=F, xlim = c(0,20)) # both axis have to be set false so that then we can overlaigh a axis with parameters we want
# # to use main title with its corresponding parameters (for ex. size) we have to set the title appart from barplot:
# #title(main = list("Biological Processes - mouse only", cex = 5))
# axis(side=1, at=seq(from=0,to = 20, by=4), lwd=7, tck = -0.02, labels=NA, line = -8) # this is for the ticks only in x axis. that's why lables=NA.side - an integer indicating the side of the graph to draw the axis (1=bottom, 2=left, 3=top, 4=right); at - a numeric vector indicating where tick marks should be drawn; lwd - line width;
# axis(side=1, at=seq(from=0,to = 20, by=4), lwd=0, cex=5, cex.axis=3.5, line = -4) # this is for the numbering only in x axis. that's why lwd=0.
# title(xlab=expression(-log[10](`adj.p-value`)), cex.lab = 5.5, line = 5.5) # adds x axis label
# dev.off()
...
# extra.human <- subset(onto.overGO.bp.h1[,c(8,3)],onto.overGO.bp.h1$Term%in%NAs.human) 
# extra.mouse <- subset(onto.overGO.bp.m1[,c(8,3)],onto.overGO.bp.m1$Term%in%NAs.mouse) 
# # put column FDR with the indication if it's from mouse or human
